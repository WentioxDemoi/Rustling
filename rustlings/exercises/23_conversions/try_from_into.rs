...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        if tuple.0 < 0 || tuple.1 < 0 || tuple.2 < 0 || tuple.0 > 255 || tuple.1 > 255 || tuple.2 > 255 {Err(IntoColorError::IntConversion)}
        else { Ok(Self { red: tuple.0 as u8, green: tuple.1 as u8, blue: tuple.2 as u8 }) }
    }
}
...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        ...
        Ok(Color { red: *arr.first().unwrap() as u8, green: *arr.get(1).unwrap() as u8, blue: *arr.get(2).unwrap() as u8 })
    }
}
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        if slice.len() != 3 {return Err(IntoColorError::BadLen);} else {
            if slice.iter().any(|u| {
            if *u < 0 || *u > 255 {
                return true
            } 
        ...
        Ok(Color { red: *slice.first().unwrap() as u8, green: *slice.get(1).unwrap() as u8, blue: *slice.get(2).unwrap() as u8 })
    }
}
